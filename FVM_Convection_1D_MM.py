...
def Ecdiscreta(dx,aP,aW,aE,Su,D,F,u,QA,QB,n):
    ...
    for i in range(n):
        ...
        elif i==n-1:            #Para el Ãºltimo nodo
            aE[i]=0
            Sp=-(2*D-F)
            Su[i]=(2*D-F)*QB            
...
def Restriction(nr):
    ...
    #Definicion de Parámetros    
     ...
     #Restricción              
        ...
        aPlh[i],aWlh[i],aElh[i],Sulh[i]=Ecdiscreta(dxlh[i],aPlh[i],aWlh[i],aElh[i],Sulh[i],D,F,u,QA,QB,nlh[i]) #Se calculan los coeficientes aW,aE,ap,Su,Sp
...
#        F_Cycle(nl)
    ...
    T=boundaries(T,nlh[0],dxlh[0],QA,QB)
...
def Multimalla_FMG(T,residual):
    ...
    Tlh[0]=boundaries(Tlh[0],nlh[0],dxlh[0],QA,QB)
...
aPlh[0],aWlh[0],aElh[0],Sulh[0]=Ecdiscreta(dxlh[0],aPlh[0],aWlh[0],aElh[0],Sulh[0],D,F,u,QA,QB,nlh[0]) #Se calculan los coeficientes aW,aE,ap,Su,Sp
