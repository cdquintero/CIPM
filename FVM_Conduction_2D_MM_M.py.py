...
acum=np.zeros([int(nxlh[0]),int(nylh[0])])
...
def Restriction(n_r):
    ...
    #Definicion de Parámetros    
        ...
        M=np.zeros([int(nxlh[i-1]),int(nylh[i])])
        N=np.zeros([int(nxlh[i]),int(nylh[i])])
       
     #Restricción              
        Itrpx[i],Itrpy[i]= M_Interpolation(nxlh[i-1],nxlh[i],nylh[i-1],nylh[i],Itrpx[i],Itrpy[i])
        Rstrx[i]=Itrpx[i].T
        Rstry[i]=Itrpy[i].T
       
        for j in range(1,nxlh[i-1]):
            M[j,:]=np.dot(Rstrx[i]/4,rlh[i-1][j])
           
        for j in range(1,nylh[i]):
            N[:,j]=np.dot(Rstry[i]/4,M[:,j])
       
        rlh[i]=N
       
        aPlh[i],aSlh[i],aNlh[i],aWlh[i],aElh[i],Sulh[i]=Ecdiscreta(aPlh[i],aSlh[i],aNlh[i],aWlh[i],aElh[i],Sulh[i],dxlh[i],dylh[i],z,Aew,Asn,kT,Tos,Ton,flux,Toe,nxlh[i],nylh[i]) #Se calculan los coeficientes aW,aE,ap,Su,Sp
        elh[i]=SOR(aPlh[i],aSlh[i],aNlh[i],aWlh[i],aElh[i],rlh[i],nxlh[i],nylh[i],elh[i],10)      
...
def Prolongation(n_p):
    ...
    for i in range(1,n_p):
        ...
        M=np.zeros([int(nxlh[n_p-i]),int(nylh[n_p-i-1])])
        N=np.zeros([int(nxlh[n_p-i-1]),int(nylh[n_p-i-1])])        
 
        for j in range(1,nylh[n_p-i]):
            M[j,:]=np.dot(Itrpy[n_p-i]/2,elh[n_p-i][j])
