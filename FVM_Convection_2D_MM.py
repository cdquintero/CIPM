...
D=Gamma/dxlh[0]; F=rho*u

xlh=[np.zeros(nxlh[0]+1)]; ylh=[np.zeros(nylh[0]+1)]
xclh=[np.zeros(nxlh[0]+2)]; yclh=[np.zeros(nylh[0]+2)]

aWlh=[(D+F/2)*np.ones([nxlh[0],nylh[0]])]; aElh=[(D-F/2)*np.ones([nxlh[0],nylh[0]])]
aSlh=[(D+F/2)*np.ones([nxlh[0],nylh[0]])]; aNlh=[(D-F/2)*np.ones([nxlh[0],nylh[0]])]
...
def Ecdiscreta(dx,dy,z,aP,aS,aN,aW,aE,Su,D,F,u,QA,QB,nx,ny):
...
def Parametros(nl,a):
    ...
    for i in range(1,nl):
        ...
        aSlh.append((D+F/2)*np.ones([int(nxlh[i]),int(nylh[i])]))
        aNlh.append((D-F/2)*np.ones([int(nxlh[i]),int(nylh[i])]))
        aWlh.append((D+F/2)*np.ones([int(nxlh[i]),int(nylh[i])]))
        aElh.append((D-F/2)*np.ones([int(nxlh[i]),int(nylh[i])]))    
...
def Restriction(n_r):
    ...
    for i in range (1,n_r):    
     ...
     #Restricción              
        ...
        aPlh[i],aSlh[i],aNlh[i],aWlh[i],aElh[i],Sulh[i]=Ecdiscreta(dxlh[i],dylh[i],z,aPlh[i],aSlh[i],aNlh[i],aWlh[i],aElh[i],Sulh[i],D,F,u,QA,QB,nxlh[i],nylh[i])       
...
#        aPlh[i-1],aSlh[i-1],aNlh[i-1],aWlh[i-1],aElh[i-1],Sulh[i-1]=Ecdiscreta(dxlh[i-1],dylh[i-1],z,aPlh[i-1],aSlh[i-1],aNlh[i-1],aWlh[i-1],aElh[i-1],Sulh[i-1],D,F,u,QA,QB,nxlh[i-1],nylh[i-1])       
...
def FMG_Cycle(nl):
    ...
    for i in range(1,nl):                
        ...
        aPlh[i],aSlh[i],aNlh[i],aWlh[i],aElh[i],Sulh[i]=Ecdiscreta(dxlh[i],dylh[i],z,aPlh[i],aSlh[i],aNlh[i],aWlh[i],aElh[i],Sulh[i],D,F,u,QA,QB,nxlh[i],nylh[i])       
 #Se calculan los coeficientes aW,aE,ap,Su,Sp
...
#        F_Cycle(nl)
    ...
    T=boundaries(T,nxlh[0],nylh[0],dxlh[0],dylh[0],QA,QB)              #Se establecen las temperaturas de las fronteras
...
def Multimalla_FMG(T,residual):
    ...
    Tlh[0]=boundaries(Tlh[0],nxlh[0],nylh[0],dxlh[0],dylh[0],QA,QB)              #Se establecen las temperaturas de las fronteras
...
aPlh[0],aSlh[0],aNlh[0],aWlh[0],aElh[0],Sulh[0]=Ecdiscreta(dxlh[0],dylh[0],z,aPlh[0],aSlh[0],aNlh[0],aWlh[0],aElh[0],Sulh[0],D,F,u,QA,QB,nxlh[0],nylh[0]) #Se calculan los coeficientes aW,aE,ap,Su,Sp
